...
{
    int knaptable_ptr[capacity + 1];
    memset(knaptable_ptr, 0, sizeof(int) * capacity + 1);
    int i, w, j, dj;
    ...
    for (i = 1; i < items_num + 1; i++){
        ...
        for (w = 0; w < capacity + 1; w++){
            if (tmp_item.weight <= w){
                int sum_value = tmp_item.value + knaptable_ptr[w-tmp_item.weight]; //summary value of itemset
                if (sum_value > knaptable_ptr[w]){
                    knaptable_ptr[w] = sum_value;//put it
